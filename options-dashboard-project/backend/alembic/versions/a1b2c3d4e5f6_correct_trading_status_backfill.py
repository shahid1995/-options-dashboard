...
does not mean trading authorized.

This corrective migration resets trading_status to 'inactive' for all
rows where it was incorrectly set to 'active' by the original backfill.
...
def upgrade() -> None:
    # Reset trading_status to 'inactive' for all rows.
    # The original backfill incorrectly set trading_status='active' for
    # status='connected' rows. Broker OAuth connected != trading authorized.
    ...
    op.execute(
        ...
        WHERE trading_status = 'active'
          AND broker_account_id != 'trading-authorized'
        """
    )
...
def downgrade() -> None:
    ...
    # trading-authorized before this correction.
    pass
