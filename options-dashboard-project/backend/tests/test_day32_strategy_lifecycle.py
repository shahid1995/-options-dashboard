"""Day 32 — Strategy lifecycle and Opportunity Gate tests (RED phase).

The tests lock the boundary before implementation:
Opportunity -> Strategy Candidate -> Strategy Evaluation -> Opportunity Gate.

Day 32 must remain pure, deterministic, broker-neutral and separate from
central risk authorization and execution.
"""
...
from app.intelligence.contracts import IntelligenceDirection
...
def _fake_evaluation(
    *,
    status: StrategyEvaluationStatus = StrategyEvaluationStatus.SUCCESS,
...
def test_missing_opportunity_is_blocked_explicitly():
    result = evaluate_strategy_gate(
        None, _fake_ranking(), _fake_evaluation(),
...
def test_incomplete_evaluation_cannot_become_eligible():
    result = evaluate_strategy_gate(
        _fake_opportunity(), _fake_ranking(),
...
def test_missing_strike_selection_is_blocked():
    ...
    result = evaluate_strategy_gate(
        _fake_opportunity(), empty, _fake_evaluation(),
        strategy_id="strategy-1", legs=(_fake_leg(),),
        expected_behavior=ExpectedBehavior.DIRECTIONAL_CONTINUATION_CANDIDATE,
        invalidation="thesis invalidated", reference_timestamp=REF,
        quality=_quality(),
...
def test_insufficient_quality_blocks_gate_without_coercion():
    result = evaluate_strategy_gate(
        _fake_opportunity(), _fake_ranking(), _fake_evaluation(),
        strategy_id="strategy-1", legs=(_fake_leg(),),
        expected_behavior=ExpectedBehavior.DIRECTIONAL_CONTINUATION_CANDIDATE,
        invalidation="thesis invalidated", reference_timestamp=REF,
...
def test_candidate_is_immutable_and_serializes_deterministically():
    ...
    assert first == second
    json.dumps(first)
    assert StrategyCandidate.from_dict(first).to_dict() == first


def test_gate_result_serialization_is_deterministic_and_preserves_provenance():
    result = evaluate_strategy_gate(
        _fake_opportunity(), _fake_ranking(), _fake_evaluation(),
        strategy_id="strategy-1", legs=(_fake_leg(),),
        expected_behavior=ExpectedBehavior.DIRECTIONAL_CONTINUATION_CANDIDATE,
    ...
    assert encoded["provenance"]["source"] == "OPPORTUNITY"
...
def test_same_inputs_are_context_equivalent():
    ...
    for context in EvaluationContext:
        ...
        results.append(evaluate_strategy_gate(
            _fake_opportunity(), _fake_ranking(), evaluation,
            strategy_id="strategy-1", legs=(_fake_leg(),),
            expected_behavior=ExpectedBehavior.DIRECTIONAL_CONTINUATION_CANDIDATE,
            invalidation="thesis invalidated", reference_timestamp=REF,
            quality=_quality(),
        ).to_dict())
    normalized = [{k: v for k, v in item.items() if k != "evaluation_context"} for item in results]
    assert all(item == normalized[0] for item in normalized)


def test_gate_has_no_execution_or_risk_authorization_surface():
    source = Path(__file__).parents[1] / "app" / "strategy_lifecycle"
    text = "\n".join(p.read_text(encoding="utf-8") for p in source.glob("*.py") if p.exists())
    forbidden = ("Order", "BrokerAdapter", "Execution", "UserApproval", "RiskDecision", "authorize")
    for token in forbidden:
        assert token not in text


def test_lifecycle_package_is_pure_domain_without_io_clock_or_random_imports():
    source = Path(__file__).parents[1] / "app" / "strategy_lifecycle"
    forbidden_modules = {"datetime", "time", "random", "uuid", "requests", "httpx", "sqlalchemy"}
