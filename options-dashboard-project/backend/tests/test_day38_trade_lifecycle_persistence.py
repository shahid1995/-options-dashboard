...
- uniqueness for deterministic lifecycle sequencing
...
# ---------------------------------------------------------------------------
# 1. Deterministic event_id
# ---------------------------------------------------------------------------

def test_event_id_is_deterministic():
    eid = event_id("execution", "exec-a", "PositionOpened", 1)
    assert eid == event_id("execution", "exec-a", "PositionOpened", 1)


def test_event_id_differs_by_aggregate_type():
    a = event_id("execution", "exec-a", "PositionOpened", 1)
    b = event_id("position", "exec-a", "PositionOpened", 1)
    assert a != b


def test_event_id_differs_by_aggregate_id():
    a = event_id("execution", "exec-a", "PositionOpened", 1)
    b = event_id("execution", "exec-b", "PositionOpened", 1)
    assert a != b


def test_event_id_differs_by_event_type():
    a = event_id("execution", "exec-a", "PositionOpened", 1)
    b = event_id("execution", "exec-a", "PositionClosed", 1)
    assert a != b


def test_event_id_differs_by_sequence():
    a = event_id("execution", "exec-a", "PositionOpened", 1)
    b = event_id("execution", "exec-a", "PositionOpened", 2)
    assert a != b
...
def test_event_stores_aggregate_id(fresh_db):
    ...
    row = db.execute(
        ...
        {"eid": event_id("execution", "exec-abc", "PositionOpened", 1)},
    ).scalar()
    assert row == "exec-abc"
    db.close()
...
def test_event_stores_sequence(fresh_db):
    ...
    row = db.execute(
        ...
        {"eid": event_id("execution", "exec-a", "PositionOpened", 5)},
    ).scalar()
    assert row == 5
    db.close()
...
def test_event_stores_position_sequence(fresh_db):
    ...
    row = db.execute(
        ...
        {"eid": event_id("position", "pos-1", "PositionOpened", 1)},
    ).scalar()
    assert row == 3
    db.close()
...
def test_event_type_and_version_stored(fresh_db):
    ...
    row = db.execute(
        ...
        {"eid": event_id("execution", "exec-a", "PositionClosed", 1)},
    ).fetchone()
...
def test_occurred_at_stored(fresh_db):
    ...
    # SQLite stores DateTime as string; compare ISO format
    row = db.execute(
...
# ---------------------------------------------------------------------------
# 11. Uniqueness for deterministic lifecycle sequencing
# ---------------------------------------------------------------------------

def test_aggregate_sequence_unique_constraint(fresh_db):
    db = _session()
    append_lifecycle_event(
        db=db,
        aggregate_type="execution",
    ...
    # Inserting the same aggregate+sequence with different content must fail
...
def test_position_sequence_anchor_scoped_by_complete_identity(fresh_db):
    ...
    # Different identity gets independent sequence
    seq2 = allocate_position_sequence(
        ...
        expiry="2026-12-31",
        strike=24000.0,
        option_type="PE",  # different option_type = different identity
    )
    assert seq2 == 1
...
# ---------------------------------------------------------------------------
# 13. Duplicate / idempotency constraints
# ---------------------------------------------------------------------------
...
def test_conflicting_duplicate_raises(fresh_db):
    ...
    with pytest.raises(IntegrityError):
        append_lifecycle_event(
            ...
            sequence=1,
            position_sequence=1,
            quantity_delta=-10,  # different delta!
            position_identity={
                "user_id": "user-1",
                "symbol": "NIFTY",
                "expiry": "2026-12-31",
...
def test_rollback_does_not_burn_sequence(fresh_db):
    """If a transaction rolls back, the allocated sequence is not burned."""
    db = _session()
    ...
    assert seq1 == 1
    db.rollback()  # rollback the allocation

    # After rollback, a new allocation should start fresh at 1
    seq2 = allocate_position_sequence(
        db=db,
        tenant_id="tenant-1",
        user_id="user-1",
...
def test_concurrent_first_event_allocation_is_safe(fresh_db):
    """Two concurrent transactions allocating the same PositionIdentity
    should not both get sequence=1.

    This is proven at the database level by the upsert; we verify the
    sequence advances correctly when called sequentially (which is what
    SQLite supports; PostgreSQL row-level locking provides the real
