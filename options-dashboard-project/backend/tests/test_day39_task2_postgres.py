...
@pytest.fixture()
def pg_db(pg_engine):
    """Fresh PostgreSQL session for each test, rolled back after."""
...
class TestPostgresDay38:
    def test_lifecycle_event_persisted_with_correct_fields(self, pg_db):
        ...
        row = pg_db.execute(
            select(TradeLifecycleEvent).limit(1)
        ).scalar_one()
        assert row.aggregate_type == "execution"
        assert row.aggregate_id == "ORD-PG-LC"
    ...
    def test_day38_sequence_independent_of_broker_sequence(self, pg_db):
        ...
        rows = pg_db.execute(
            ...
            .where(TradeLifecycleEvent.aggregate_id == "ORD-PG-SEQ-IND")
    ...
    def test_day38_replay_compatibility(self, pg_db):
        ...
        rows = pg_db.execute(
            ...
            .where(TradeLifecycleEvent.aggregate_id == order_id)
            .order_by(TradeLifecycleEvent.sequence.asc())
        ).scalars().all()
        assert len(rows) == 4
