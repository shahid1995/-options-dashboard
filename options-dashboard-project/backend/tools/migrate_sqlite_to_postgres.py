...
def redact_url(url:str)->str:
    ...
    except Exception: return "<redacted database url>"
...
def _fingerprint_token(value:Any)->tuple[str,...]:
    ...
    if isinstance(value,datetime):
        # SQLite may return naive UTC while PostgreSQL returns aware UTC.
        # Fingerprints compare the same instant without changing stored values.
...
def get_alembic_heads(alembic_dir:str|Path)->list[str]:
    ...
    directory=Path(alembic_dir).resolve(); cfg=Config(str(directory.parent/"alembic.ini")); cfg.set_main_option("script_location",str(directory))
    return list(ScriptDirectory.from_config(cfg).get_heads())

def backup_sqlite(source_path:str,destination_path:str)->None:
    source=Path(source_path).resolve(); destination=Path(destination_path).resolve()
    if not source.exists():raise FileNotFoundError(source)
    destination.parent.mkdir(parents=True,exist_ok=True)
    with sqlite3.connect(str(source),timeout=30) as src:
        if src.execute("PRAGMA quick_check").fetchone()[0]!="ok":raise RuntimeError("SQLite integrity check failed")
...
class SQLiteReader:
    def __init__(self,db_path:str):
        ...
        self.conn=sqlite3.connect(f"file:{self.db_path}?mode=ro",uri=True,timeout=30); self.conn.row_factory=sqlite3.Row
    ...
    def get_pk_columns(self,table):
        rows=self.conn.execute(f"PRAGMA table_info([{table}])").fetchall(); return [r[1] for r in sorted(rows,key=lambda r:r[5]) if r[5]>0]
    def count(self,table):return int(self.conn.execute(f"SELECT COUNT(*) FROM [{table}]").fetchone()[0])
    def fetch_all(self,table,columns):
        cols=", ".join(f"[{c}]" for c in columns); return [tuple(r) for r in self.conn.execute(f"SELECT {cols} FROM [{table}]")]
    def fetch_batch(self,table,columns,offset,limit):
        cols=", ".join(f"[{c}]" for c in columns); return [tuple(r) for r in self.conn.execute(f"SELECT {cols} FROM [{table}] LIMIT ? OFFSET ?",(limit,offset))]
...
class PgWriter:
    ...
    def table_exists(self,table):
        with self.conn.cursor() as c:c.execute("SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name=%s)",(table,)); return bool(c.fetchone()[0])
    def get_columns(self,table):
        with self.conn.cursor() as c:c.execute("SELECT column_name FROM information_schema.columns WHERE table_schema='public' AND table_name=%s ORDER BY ordinal_position",(table,)); return [r[0] for r in c.fetchall()]
    ...
    def get_not_null_columns(self,table):
        with self.conn.cursor() as c:c.execute("SELECT column_name FROM information_schema.columns WHERE table_schema='public' AND table_name=%s AND is_nullable='NO'",(table,)); return [r[0] for r in c.fetchall()]
    ...
    def check_fk_integrity(self,table):
        ...
        for col,ref_table,ref_col in self.get_fk_constraints(table):
            with self.conn.cursor() as c:c.execute(f'SELECT COUNT(*) FROM "{table}" t LEFT JOIN "{ref_table}" r ON t."{col}"=r."{ref_col}" WHERE t."{col}" IS NOT NULL AND r."{ref_col}" IS NULL'); n=int(c.fetchone()[0])
            if n:errors.append(f"{table}.{col} -> {ref_table}.{ref_col}: {n} orphaned rows")
        return errors
    def check_not_null(self,table):
        errors=[]
        for col in self.get_not_null_columns(table):
            with self.conn.cursor() as c:c.execute(f'SELECT COUNT(*) FROM "{table}" WHERE "{col}" IS NULL'); n=int(c.fetchone()[0])
            if n:errors.append(f"{table}.{col}: {n} NULL values")
        return errors
    def get_sequence_info(self):
        with self.conn.cursor() as c:
            c.execute("SELECT table_name,column_name,pg_get_serial_sequence('public.' || table_name,column_name) FROM information_schema.columns WHERE table_schema='public' AND (is_identity='YES' OR column_default LIKE 'nextval(%') ORDER BY table_name,column_name"); return [(r[0],r[1],r[2]) for r in c.fetchall() if r[2]]
    ...
    def verify_security_invariants(self):
        with self.conn.cursor() as c:
            c.execute("SELECT COUNT(*) FROM users"); users=int(c.fetchone()[0]); c.execute("SELECT COUNT(*) FROM user_sessions"); sessions=int(c.fetchone()[0]); c.execute("SELECT COUNT(*) FROM broker_connections"); conns=int(c.fetchone()[0]); c.execute("SELECT COUNT(*) FROM broker_tokens"); tokens=int(c.fetchone()[0]); c.execute("SELECT DISTINCT data_source FROM gex_snapshots WHERE data_source IS NOT NULL ORDER BY data_source"); sources=[r[0] for r in c.fetchall()]; c.execute("SELECT COUNT(*) FROM gex_snapshots WHERE data_source IN ('analytics_token','broker_oauth') AND connection_id IS NULL"); missing=int(c.fetchone()[0]); c.execute("SELECT trading_status,COUNT(*) FROM broker_connections GROUP BY trading_status"); trading={r[0]:int(r[1]) for r in c.fetchall()}
        return {"users_count":users,"user_sessions_count":sessions,"broker_connections_count":conns,"broker_tokens_count":tokens,"gex_data_sources":sources,"invalid_gex_sources":[s for s in sources if s not in GEX_DATA_SOURCES],"gex_missing_connection_provenance":missing,"trading_status":trading}
    def verify_multi_user_isolation(self,user_ids):
        results={}
        with self.conn.cursor() as c:
            for uid in user_ids:
                c.execute("SELECT COUNT(*) FROM user_sessions WHERE user_id=%s",(uid,)); sessions=int(c.fetchone()[0]); c.execute("SELECT COUNT(*) FROM broker_connections WHERE user_id=%s",(uid,)); conns=int(c.fetchone()[0]); c.execute("SELECT COUNT(*) FROM gex_snapshots WHERE owner_id=%s",(uid,)); gex=int(c.fetchone()[0]); results[uid]={"sessions":sessions,"connections":conns,"gex_snapshots":gex}
        return results

def _order_table_names(metadata,names=None):
    selected={n:t for n,t in metadata.tables.items() if n not in SKIP_TABLES and (names is None or n in names)}; deps={n:set() for n in selected}
...
def assert_schema_compatible(source_metadata,target_metadata):
    source_tables={n for n in source_metadata.tables if n not in SKIP_TABLES}; target_tables={n for n in target_metadata.tables if n not in SKIP_TABLES}; missing=sorted(source_tables-target_tables)
...
def migrate_table(reader,writer,table,dry_run=False):
    r=MigrationResult(table=table); start=time.monotonic(); r.source_count=reader.count(table)
    if r.source_count==0:r.skipped=True;r.skip_reason="empty source table";return r
    if not writer.table_exists(table):r.error=f"Table {table} does not exist in PostgreSQL";return r
    common=[c for c in reader.get_columns(table) if c in writer.get_columns(table)]
    if not common:r.error=f"No common columns for {table}";return r
    if dry_run:r.skipped=True;r.skip_reason="dry run";return r
    offset=0
    while offset<r.source_count:
        batch=reader.fetch_batch(table,common,offset,BATCH_SIZE)
        if not batch:break
        r.rows_written+=writer.insert_batch(table,common,batch); offset+=len(batch)
    r.target_count=writer.count(table); r.duration_seconds=time.monotonic()-start; return r
...
def migrate_database(sqlite_engine,postgres_engine,batch_size=BATCH_SIZE):
    ...
    sm=MetaData(); tm=MetaData(); sm.reflect(bind=sqlite_engine); tm.reflect(bind=postgres_engine); assert_schema_compatible(sm,tm); names={n for n in sm.tables if n not in SKIP_TABLES}; order=_order_table_names(tm,names); src=sqlite_engine.connect(); dst=postgres_engine.connect(); tx=dst.begin()
    try:
        for table_name in order:
            st=sm.tables[table_name]; tt=tm.tables[table_name]; common=[c.name for c in st.columns if c.name in tt.columns]
            if not common:continue
            result=src.execute(select(*[st.c[n] for n in common]))
            while True:
                rows=result.fetchmany(batch_size)
                if not rows:break
                dst.execute(tt.insert(),[dict(zip(common,row)) for row in rows])
...
def verify_databases(sqlite_engine,postgres_engine):
    ...
    if not path or path==":memory:":raise ValueError("verify_databases requires a file-backed SQLite database")
    reader=SQLiteReader(str(path)); writer=PgWriter.from_sqlalchemy_engine(postgres_engine)
    try:
        tables=[t for t in reader.get_tables() if t not in SKIP_TABLES]; vers=[verify_table(reader,writer,t) for t in tables if writer.table_exists(t)]; seq=writer.check_sequences(); sec=writer.verify_security_invariants(); ids=[]
        with writer.conn.cursor() as c:c.execute("SELECT id FROM users ORDER BY id"); ids=[r[0] for r in c.fetchall()]
        iso=writer.verify_multi_user_isolation(ids)
        return {"ok":all(v.passed for v in vers) and all(x.get("ok",False) for x in seq.values()) and not sec.get("invalid_gex_sources") and int(sec.get("gex_missing_connection_provenance",0))==0 and "cross_user_violation" not in iso,"tables":{v.table:{"row_count":v.target_count,"source_count":v.source_count,"fingerprint_match":v.fingerprint_match,"source_fingerprint":v.source_fingerprint,"target_fingerprint":v.target_fingerprint,"pk_unique":v.pk_unique,"fk_clean":v.fk_clean,"not_null_clean":v.not_null_clean,"errors":v.errors,"passed":v.passed} for v in vers},"sequences":seq,"security":sec,"isolation":iso}
...
def main()->int:
    ...
    try:
        ...
        if args.validate_only or args.ready_for_cutover:
            vers=[verify_table(reader,writer,t) for t in tables if writer.table_exists(t)]; sec=writer.verify_security_invariants();
            with writer.conn.cursor() as c:c.execute("SELECT id FROM users ORDER BY id"); ids=[r[0] for r in c.fetchall()]
        ...
        try:report=migrate_database(src_engine,pg_engine,batch_size=BATCH_SIZE)
