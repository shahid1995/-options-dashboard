...
def storage_safety_ok(source_size: int, target_capacity: int) -> bool:
    return (
        ...
        and source_size <= int(target_capacity * 0.8)
    )
...
def _fingerprint_token(value: Any) -> tuple[str, ...]:
    ...
    if isinstance(value, str):
        # SQLite commonly stores SQLAlchemy DateTime values as text, while
        # PostgreSQL returns datetime objects. Normalize only timestamp-like
        # strings so ordinary text identifiers are left untouched.
        timestamp_like = (
            len(value) >= 19
            and value[4] == "-"
            and value[7] == "-"
...
def sha256_rows(rows: Iterable[Sequence[Any]]) -> str:
    ...
    payload = "\n".join(
        "|".join(":".join(part) for part in row) for row in ordered
    )
    ...
    with sqlite3.connect(str(source), timeout=30) as src:
        ...
        with sqlite3.connect(str(destination), timeout=30) as dst:
            ...
            if backup_integrity != "ok":
                raise RuntimeError(
                    f"Backup integrity check failed: {backup_integrity}"
                )
...
class SQLiteReader:
    ...
    def wal_checkpoint(self) -> None:
        raise RuntimeError(
            "WAL checkpoint requires a read-write SQLite connection; "
            "use backup_sqlite()"
        )
    ...
    def fetch_all(self, table: str, columns: list[str]) -> list[tuple]:
        ...
        return [
            tuple(row)
            for row in self.conn.execute(f"SELECT {cols} FROM [{table}]")
        ]

    def fetch_batch(
        self, table: str, columns: list[str], offset: int, limit: int
    ) -> list[tuple]:
        cols = ", ".join(f"[{column}]" for column in columns)
        return [
            tuple(row)
...
class PgWriter:
    ...
    def __init__(
        ...
        if connection is None:
            ...
            self.conn = psycopg.connect(
                normalize_url(pg_url), connect_timeout=15
            )
    ...
    def get_pk_columns(self, table: str) -> list[str]:
        with self.conn.cursor() as cur:
            cur.execute(
                ...
                "k(attnum, ordinality) ON k.attnum=a.attnum "
    ...
    def check_pk_uniqueness(
        ...
        with self.conn.cursor() as cur:
            cur.execute(
                f'SELECT COUNT(*) FROM '
                f'(SELECT DISTINCT {cols} FROM "{table}") t'
            )
    ...
    def verify_security_invariants(self) -> dict[str, Any]:
        with self.conn.cursor() as cur:
            ...
            cur.execute(
                "SELECT COUNT(*) FROM gex_snapshots "
                "WHERE data_source IN ('analytics_token','broker_oauth') "
                "AND connection_id IS NULL"
            )
            missing_connection = int(cur.fetchone()[0])
            cur.execute(
                "SELECT trading_status, COUNT(*) FROM broker_connections "
        ...
        return {
            ...
            "invalid_gex_sources": [
                source for source in sources if source not in GEX_DATA_SOURCES
            ],
    ...
    def verify_multi_user_isolation(
        ...
        with self.conn.cursor() as cur:
            for user_id in user_ids:
                ...
                sessions = int(cur.fetchone()[0])
                cur.execute(
                    "SELECT COUNT(*) FROM broker_connections WHERE user_id=%s",
                    (user_id,),
                )
                connections = int(cur.fetchone()[0])
                cur.execute(
                    "SELECT COUNT(*) FROM gex_snapshots WHERE owner_id=%s",
                    (user_id,),
                )
...
def _order_table_names(
    metadata, names: set[str] | None = None
) -> list[str]:
    selected = {
        table_name: table
        for table_name, table in metadata.tables.items()
        if table_name not in SKIP_TABLES
        and (names is None or table_name in names)
    }
    dependencies: dict[str, set[str]] = {
        table_name: set() for table_name in selected
    }
    ...
    while dependencies:
        ready = sorted(
            table_name
            for table_name, parents in dependencies.items()
            if not parents
        )
        if not ready:
        ...
        for table_name in ready:
            dependencies.pop(table_name, None)
...
def assert_schema_compatible(source_metadata, target_metadata) -> None:
    ...
    if missing:
        raise ValueError(
            "tables missing from target: " + ", ".join(missing)
        )

    for table_name in sorted(source_tables):
        source_columns = set(
            source_metadata.tables[table_name].columns.keys()
        )
        target_columns = set(
            target_metadata.tables[table_name].columns.keys()
        )
        missing_columns = sorted(source_columns - target_columns)
        if missing_columns:
            raise ValueError(
                f"columns missing from target for {table_name}: "
...
def assert_target_empty(writer: PgWriter, tables: Sequence[str]) -> None:
    ...
    if non_empty:
        raise RuntimeError(
            ...
            + ", ".join(f"{table}={count}" for table, count in non_empty)
        )
...
def migrate_table(
    ...
    while offset < result.source_count:
        batch = reader.fetch_batch(
            table, common_columns, offset, BATCH_SIZE
        )
        if not batch:
            break
        result.rows_written += writer.insert_batch(
            table, common_columns, batch
        )
        offset += len(batch)
...
def migrate_database(
    sqlite_engine, postgres_engine, batch_size: int = BATCH_SIZE
) -> dict[str, Any]:
    ...
    target_conn = postgres_engine.connect()
...
def verify_table(
    reader: SQLiteReader, writer: PgWriter, table: str
) -> VerificationResult:
    ...
    common = [
        ...
        if column in writer.get_columns(table)
    ]
    if not common:
        result.errors.append("no common columns")
        return result

    result.source_fingerprint = reader.compute_fingerprint(
        table, common
    )
    result.target_fingerprint = writer.compute_fingerprint(
        table, common
    )
    result.fingerprint_match = (
        result.source_fingerprint == result.target_fingerprint
    )
    result.pk_unique = writer.check_pk_uniqueness(
        table, writer.get_pk_columns(table)
    )[0]
    ...
    not_null_errors = writer.check_not_null(table)
    result.not_null_clean = not not_null_errors
    result.errors.extend(not_null_errors)
...
def verify_databases(sqlite_engine, postgres_engine) -> dict[str, Any]:
    ...
    if not source_path or source_path == ":memory:":
        raise ValueError(
            "verify_databases requires a file-backed SQLite database"
        )
    ...
    try:
        ...
        verifications = [
            verify_table(reader, writer, table)
            for table in tables
            if writer.table_exists(table)
        ]
        ...
        return {
            "ok": (
                ...
                and int(
                    security.get(
                        "gex_missing_connection_provenance", 0
                    )
                )
                == 0
...
def check_ready_for_cutover(
    ...
    if len(heads) != 1:
        ...
        try:
            with writer.conn.cursor() as cur:
                ...
                row = cur.fetchone()
            if not row or row[0] != heads[0]:
                reasons.append(
                    "Alembic database head does not match repository head"
                )
        except Exception:
            reasons.append("Alembic head could not be verified")

    for result in results:
        if result.error:
            reasons.append(f"Migration error in {result.table}")
    for verification in verifications:
        if not verification.passed:
            reasons.append(
                f"Verification failed for {verification.table}"
            )
    ...
    if security.get("gex_missing_connection_provenance"):
        reasons.append(
            "User-owned GEX snapshot is missing connection provenance"
        )
    if "cross_user_violation" in user_isolation:
        reasons.append("Cross-user ownership violation")
    if not all(
        item.get("ok", False)
        for item in writer.check_sequences().values()
    ):
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Safe SQLite -> PostgreSQL migration for StrikeNova"
    )
    parser.add_argument(
        "--sqlite", required=True, help="Path to a verified SQLite backup"
    )
    parser.add_argument(
        "--pg-url",
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate inputs without database writes",
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="Verify an already migrated target",
    )
    parser.add_argument(
        "--ready-for-cutover",
        action="store_true",
        help="Run readiness checks only",
    )
    args = parser.parse_args()

    pg_url = normalize_url(
        args.pg_url or os.getenv("DATABASE_URL") or ""
    )
    if not pg_url.startswith("postgresql+psycopg://"):
        print(
            "ERROR: PostgreSQL URL must use the supported "
            "psycopg dialect"
        )
        return 1
    ...
    try:
        ...
        if args.dry_run:
            print(
                "DRY RUN: source validated; no PostgreSQL writes performed"
            )
            return 0
