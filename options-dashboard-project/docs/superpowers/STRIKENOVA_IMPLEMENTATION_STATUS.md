...
| Baseline | `1cf0da4828f899dbee8778eacb6a5acaab1cdc06` (approved Day-35 HEAD) on branch `feat/strikenova-day35-portfolio-intelligence` (the approved Day-36 work branch; no separate branch was created) |
| Implementation SHA | `484a67f` (baseline `1cf0da4` = approved Day-35 HEAD) |
| Note on plan doc | The approved Day-36 implementation-plan file `docs/superpowers/plans/2026-09-05-strikenova-day36-final-risk-gate.md` does NOT exist in-tree (the design was authorized in-conversation, mirroring Day 34); the master plan's Day-36 section ("Risk gate … Gate: Risk Gate PASS") is generic and sets no thresholds. The implementation therefore follows the authorization spec and the Day-33 architectural precedent, and invents NO thresholds |
| Module | `app/final_risk_gate/` — `contracts.py` (immutable contracts + explicit JSON `to_dict`/`from_dict`), `gate.py` (`evaluate_final_gate(candidate, central_risk, portfolio, *, policy, reference_timestamp=None, tenant_id=None)`), `__init__.py`. Pure domain: no DB/network/filesystem/broker/wall-clock/random; `datetime` used only as the caller-supplied reference type (annotation + ISO parse/serialize) |
...
| Rules | `CENTRAL_RISK_PASS` (always evaluated) + `MAX_PORTFOLIO_DELTA` / `MAX_PROJECTED_DELTA` / `MAX_CONCENTRATION_SHARE` — evaluated ONLY when their cap is explicitly configured (`None` = rule not configured, Day-33 `RiskPolicy` precedent; no threshold is invented). Rule outcomes use Day-33 semantics: `passed=True/False/None` (unverifiable never = safe). Deterministic ladder: INVALID > BLOCKED > UNAVAILABLE > PARTIAL > PASS; dimensions (A-G structural/central/portfolio-impact/concentration/directional/regime/data-quality) are informational assessments with their own states (regime unknown → UNAVAILABLE dimension + `REGIME_UNKNOWN` issue, never a direction guess) |
