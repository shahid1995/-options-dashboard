...
function Metric({ label, value, accent = false, note }) {
  return (
    <div>
      <div style={{ fontSize: "0.625rem", fontWeight: 600, letterSpacing: "0.06em", color: COLOR.textFaint, marginBottom: SPACE.xs }}>{label}</div>
      <div style={{ fontSize: "1.125rem", fontWeight: 700, color: accent ? COLOR.strategy : COLOR.textPrimary, fontFamily: TYPE.data }}>{value}</div>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: SPACE.xs }}>
            {["Bullish bias", "Risk capped", "1.22 : 1 R/R"].map((item, i) => (
              <span key={item} style={{ display: "inline-flex", padding: `${SPACE.xs} ${SPACE.small}`, border: `1px solid ${COLOR.borderSubtle}`, color: i === 2 ? COLOR.strategy : COLOR.textSecondary, fontFamily: TYPE.data, fontSize: "0.6875rem", borderRadius: RADIUS.sm }}>{item}</span>
            ))}
          </div>
        </div>
      </div>
        ...
        <div>
          ...
          <div style={{ width: "100%", overflow: "hidden" }}>
            <svg viewBox="0 0 620 250" width="100%" height="250" role="img" aria-label="Illustrative bull call spread payoff. Maximum loss is negative 45 points, breakeven is 25,495, short strike is 25,550, and maximum profit is positive 55 points.">
              ...
              <path d="M48 170 L160 170 L286 125 L48 125 Z" fill={COLOR.negative} opacity="0.09" />

              <path d="M286 125 L440 70 L590 70" fill="none" stroke={COLOR.positive} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
              <path d="M286 125 L440 70 L590 70 L590 125 L286 125 Z" fill={COLOR.positive} opacity="0.09" />
...
export default function WorkflowTabs({ steps, defaultStep, ariaLabel }) {
  ...
  const displayedContent = isStrategyWorkflow
    ? { market: <StrategyMarketPanel />, strategy: <StrategyStructurePanel />, payoff: <StrategyPayoffPanel />, risk: <StrategyRiskPanel /> }[activeStep]
    : activeContent;

  return (
    <div>
      <div role="tablist" aria-label={ariaLabel} style={{ display: "flex", gap: 0, marginBottom: SPACE.cardLg, flexDirection: isMobile ? "column" : "row" }}>
            ...
            <React.Fragment key={step.id}>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: SPACE.small, width: "100%" }}>
                  <span style={{ fontSize: "0.625rem", fontWeight: 700, fontFamily: TYPE.data, color: isActive ? COLOR.strategy : COLOR.textFaint, letterSpacing: "0.04em" }}>{step.number}</span>
                  <span style={{ fontSize: TYPE.caption.size, fontWeight: 700, letterSpacing: "0.06em", fontFamily: TYPE.data, textTransform: "uppercase", color: isActive ? COLOR.textPrimary : COLOR.textMuted }}>{step.label}</span>
                </div>
                <span style={{ fontSize: "0.6875rem", color: isActive ? COLOR.textSecondary : COLOR.textFaint, lineHeight: 1.4 }}>{step.sublabel}</span>
              </button>
              {!isLast && !isMobile && (
                <div style={{ display: "flex", alignItems: "center", paddingBottom: "2rem", flexShrink: 0 }} aria-hidden="true">
                  ...
                  <svg width="6" height="8" viewBox="0 0 6 8" fill="none" style={{ marginLeft: -1 }} aria-hidden="true"><path d="M0 0 L6 4 L0 8 Z" fill={COLOR.borderSubtle} /></svg>
                </div>
              )}
            </React.Fragment>
          );
      ...
      <div style={{ textAlign: "center", marginTop: SPACE.cardLg }}><DemoLabel style={{ fontSize: "0.625rem" }} /></div>
    </div>
  );
}
